...
- **transcode/transcode_mqtt.py** runs as a service, subscribes to the MQTT topic `media/rip/done`, queues each incoming path event, and transcodes every `.mkv` inside to `SERIES_DST_BASE` (default `/media/Serien`) or `MOVIE_DST_BASE` via `ffmpeg` + VAAPI hardware acceleration. By default Blu-ray audio is re-encoded to EAC3 (multichannel) or AAC (stereo) while DVDs are copied (`AUDIO_MODE=auto`); languages can be filtered via `AUDIO_LANGS`/`SUB_LANGS`. Progress and errors are published on `media/transcode/start`, `media/transcode/done`, or `media/transcode/error`.
